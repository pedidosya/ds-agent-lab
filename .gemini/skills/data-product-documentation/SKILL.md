...
- **UTF-8 Encoding and Accents Guardrail (MANDATORY):** To prevent accent marks and tildes from showing up as raw entities or being garbled, NEVER publish HTML character entities like `&amp;oacute;` or `&oacute;`, `&amp;iacute;` or `&iacute;`, `&amp;aacute;` or `&aacute;`, `&amp;eacute;` or `&eacute;`, `&amp;uacute;` or `&uacute;`, `&amp;ntilde;` or `&ntilde;`, `&amp;iquest;` or `&iquest;`, or `&amp;iexcl;` or `&iexcl;`. ALWAYS use real, standard, native UTF-8 Spanish characters directly (`ó`, `í`, `á`, `é`, `ú`, `ñ`, `¿`, `¡`). Ensure all text undergoes decoding / unescaping (e.g., via Python's `html.unescape()`) before sending the final body payload to Confluence.
- **Inline List Items Constraint (MANDATORY):** To prevent list items from sliding down onto a new line below their bullet points or numbering icons (which creates an ugly, fragmented layout), list items (`<li>`) MUST NOT contain block-level paragraph `<p>` tags. Instead, write inline text or inline markup (like `<strong>`, `<code>`, `<a>`) directly within the `<li>` tag (e.g., `<li><strong>Clave:</strong> Descripción</li>`). Strip out any block-level `<p>` and `</p>` tags nested directly inside `<li>` elements before calling the publication or update tools.
- **Confluence Schema and Macro Cleanliness (MANDATORY):** When creating or updating a Confluence page via the HTML API, never include unsupported Confluence storage-format macro tags (like `<ac:structured-macro>`, `<ac:plain-text-body>`, or XML-like structures) as they fail schema validation. Convert all code blocks to clean, native HTML preformatted containers (`<pre data-wrap="true" data-hide-line-numbers="false"><code class="language-bash">...</code></pre>`). Additionally, strip out any unsupported custom attributes such as `local-id`, `ac:local-id`, `data-table-width`, or other non-standard attributes before submitting the HTML payload.
- **Strict Indentation and Layout Validation (MANDATORY):** To ensure the published content has perfect visual formatting and is free of alignment or indentation issues, the agent MUST perform a dedicated validation of all structural elements (including HTML tables, lists, nested panels, and pre/code blocks) before calling the Confluence update or creation tools. The agent must verify that:
  - Every opening tag has a corresponding, properly positioned closing tag (perfectly balanced HTML structure).
  - Indentation inside block elements (especially `<table>`, `<ul>/<li>`, and layout sections) is clean, consistent, and logically nested.
  - Preformatted blocks (`<pre><code>`) containing ASCII/Unicode flowcharts or code snippets are precisely aligned with correct visual spacing. Spacing inside `<pre>` tags must be kept intact and free of irregular wrapping, truncation, or unwanted leading/trailing indentation that could distort the final render on Confluence.
  - Sibling block tags do not create accidental visual breaks or carriage-return irregularities.
