...
3. **FIRST QUESTION IN UPDATE MODE (MANDATORY):** If `confluence_mode` is set to `update`, the agent MUST ask the user as the very first question *before analyzing or reading the repository*: **"¿Desea actualizar algo puntual en la documentación o prefiere que lea todo el repositorio para detectar cambios automáticamente?"**.
